...
export function saveDesignToStorage(design: Omit<StoredDesign, 'id' | 'lastModified'>): string {
  try {
    ...
    const designKeys = Object.keys(storage.designs).sort((a, b) => {
      return new Date(storage.designs[b].lastModified).getTime() - 
...
export function updateDesignInStorage(designId: string, updates: Partial<StoredDesign>): void {
  try {
    const storage = getStoredDesigns();
