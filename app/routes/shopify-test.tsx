...
export default function ShopifyTest() {
  ...
  const testAddToCart = (product: any, variant: any) => {
    addTestResult(`Testing add to cart for ${product.title} - ${variant.title}`);
    console.log('Test add to cart:', { product: product.title, variant: variant.title, variantId: variant.id });
  };

  return (
    <div className="min-h-screen bg-background p-8">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-3xl font-bold text-primary mb-8">Shopify Cart Functionality Test</h1>
        
        {/* Cart Status */}
        <div className="bg-gray-900/50 border border-primary/20 rounded-sm p-6 mb-8">
          <h2 className="text-xl font-bold text-white mb-4">Current Cart Status</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-white">
            <div>
              <span className="text-gray-400">Total Items:</span>
              <span className="ml-2 font-bold text-primary">{cart?.totalQuantity || 0}</span>
            </div>
            <div>
              <span className="text-gray-400">Cart ID:</span>
              <span className="ml-2 font-mono text-sm">{cart?.id ? 'Present' : 'None'}</span>
            </div>
            <div>
              <span className="text-gray-400">Lines Count:</span>
              <span className="ml-2 font-bold text-primary">{cart?.lines?.nodes?.length || 0}</span>
            </div>
          </div>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {products.map((product: any) => {
              ...
              return (
                <div key={product.id} className="bg-gray-900/50 border border-primary/20 rounded-sm p-4">
                  ...
                  {product.variants.nodes.length > 1 && (
                    <div className="mt-4">
                      <h4 className="text-white text-sm font-bold mb-2">Other Variants:</h4>
                      <div className="space-y-2">
                        {product.variants.nodes.slice(1, 3).map((variant: any) => (
                          <div key={variant.id} className="flex items-center justify-between">
                            <span className="text-gray-400 text-sm">{variant.title}</span>
                            {variant.availableForSale && (
                              <AddToCartButton
                                ...
                                onClick={() => testAddToCart(product, variant)}
        ...
        {cart && cart.lines && cart.lines.nodes.length > 0 && (
          <div className="mb-8">
            <h2 className="text-xl font-bold text-white mb-4">Current Cart Contents</h2>
        ...
        <div className="bg-gray-900/50 border border-primary/20 rounded-sm p-6">
          <h2 className="text-xl font-bold text-white mb-4">Test Results Log</h2>
          <div className="bg-black/50 rounded-sm p-4 max-h-64 overflow-y-auto">
            {testResults.length === 0 ? (
              <p className="text-gray-400 italic">No test actions yet. Try adding items to cart.</p>
            ) : (
        ...
        <div className="mt-8 bg-blue-900/20 border border-blue-500/30 rounded-sm p-6">
          <h2 className="text-xl font-bold text-blue-400 mb-4">Testing Instructions</h2>
          <div className="text-gray-300 space-y-2">
            <p>1. <strong>Add to Cart:</strong> Click "Add to Cart" buttons to test adding items</p>
            <p>2. <strong>View Cart:</strong> Click "Open Cart Drawer" to see the cart sidebar</p>
            <p>3. <strong>Update Quantities:</strong> Use +/- buttons in cart to test quantity updates</p>
            <p>4. <strong>Remove Items:</strong> Use trash icon to test item removal</p>
            <p>5. <strong>Check Console:</strong> Open browser dev tools to see detailed logs</p>
          </div>
        </div>
      </div>
    </div>
