...
export function CartAside({ children, heading = "CART" }: { children: React.ReactNode, heading?: string }) {
  ...
  return (
    <div
      ...
      className={`fixed inset-0 z-50 transition-all duration-500 ease-out ${expanded ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
        }`}
      role="dialog"
    >
      {/* Background overlay */}
      <div
        className={`absolute inset-0 bg-gradient-to-r from-black/60 via-black/50 to-black/70 backdrop-blur-md transition-all duration-500 ${expanded ? 'opacity-100' : 'opacity-0'
          }`}
        onClick={close}
      ></div>
