...
export async function loader({params, context}: LoaderFunctionArgs) {
  ...
  try {
    ...
    response = await fetch(
      `${KLING_API_BASE}/v1/images/kolors-virtual-try-on/${taskId}`,
      {
        method: 'GET',
        headers: {
    ...
    if (!response.ok || klingResult.code !== 0) {
      response = await fetch(
        `${KLING_API_BASE}/v1/images/generations/${taskId}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
    ...
    if (taskData.task_status === 'succeed' && taskData.task_result?.images) {
      if (taskData.task_result.images.length === 1) {
        ...
        responseData.resultUrls = taskData.task_result.images.map(
          (img) => img.url,
        );
