...
export default function ProductDebug() {
  ...
  const productsWithCustomTags = products.filter(product => 
    product.tags && 
  ...
  const customizableProducts = products.filter(product => {
    const hasCustomTag = product.tags && 
      product.tags.some((tag: string) => 
        tag.toLowerCase().includes('custom') || 
        tag.toLowerCase().includes('customiz')
      );
  ...
  return (
    <div className="py-16 bg-black min-h-screen">
      <div className="container mx-auto px-4">
        <h1 className="text-3xl font-bold text-white mb-6">Product Debug Information</h1>
        
        <div className="mb-8 bg-gray-900/80 p-6 rounded-lg">
          <h2 className="text-xl font-bold text-white mb-2">Summary</h2>
          <p className="text-gray-300 mb-2">Total Products: {products.length}</p>
          <p className="text-gray-300 mb-2">Products with custom tags: {productsWithCustomTags.length}</p>
          <p className="text-gray-300 mb-2">Products with custom variants: {productsWithCustomVariants.length}</p>
          <p className="text-gray-300 mb-2">Total customizable products: {customizableProducts.length}</p>
        </div>
          ...
          <div className="grid gap-4">
            {products.map((product, index) => (
              <div key={product.id} className="bg-gray-900/80 p-4 rounded-lg">
                <h3 className="text-lg font-bold text-white mb-2">{index + 1}. {product.title}</h3>
                
                {/* Tags */}
                <div className="mb-2">
                  <h4 className="text-md font-medium text-primary mb-1">Tags:</h4>
                ...
                {/* Variants */}
                <div>
                  <h4 className="text-md font-medium text-primary mb-1">Variants:</h4>
                  {product.variants.nodes.length > 0 ? (
                    <div className="space-y-2">
                      {product.variants.nodes.map((variant, i) => (
                        <div key={i} className="bg-gray-800 px-2 py-1 rounded text-sm text-gray-300">
                          {variant.title}
                        </div>
                      ))}
                    </div>
                ...
                {/* Customizable status */}
                <div className="mt-3">
                  <h4 className="text-md font-medium text-primary mb-1">Customizable Status:</h4>
                  <div className="flex gap-4">
                    <div className="flex items-center">
                      <span className="mr-2">By Tag:</span>
                      {productsWithCustomTags.some(p => p.id === product.id) ? (
                    ...
                    <div className="flex items-center">
                      ...
                      {productsWithCustomVariants.some(p => p.id === product.id) ? (
                        <span className="text-green-500">✓</span>
                      ) : (
                        <span className="text-red-500">✗</span>
                      )}
