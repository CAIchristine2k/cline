...
async function generateKlingTokenExact(
  ak: string,
  sk: string,
): Promise<string> {
  if (!ak || !sk) {
...
async function generateKlingTokenAlternative(
  ak: string,
  sk: string,
): Promise<string> {
...
async function testKlingEndpoint(
  url: string,
  token: string,
  method: string = 'GET',
  body?: any,
  ...
  try {
    ...
    console.log(
      `📊 ${testName || 'Response'} ${response.status} ${response.statusText}:`,
      responseData,
    );
...
export async function loader({context}: LoaderFunctionArgs) {
  try {
    ...
    const token1 = await generateKlingTokenExact(
      KLING_ACCESS_KEY,
      KLING_SECRET_KEY,
    );
    const token2 = await generateKlingTokenAlternative(
      KLING_ACCESS_KEY,
      KLING_SECRET_KEY,
    );
    ...
    return new Response(
      JSON.stringify({
        ...
        summary: {
          ...
          differentResults:
            testResults.some((test) => test.success) &&
            testResults.some((test) => !test.success),
        },
        nextSteps: [
